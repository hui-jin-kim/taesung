...
function resolveKakaoRedirectUri(requested?: string) {
...
type Kind = "sale" | "jeonse" | "wolse" | "presale";
type MatchListingDoc = {
  type?: string;
  area_py?: number;
  price?: number;
  deposit?: number;
...
export const pruneViewerScenariosDaily = onSchedule(
  ...
  async () => {
    ...
    for (const doc of snap.docs) {
      const userId = (doc.data().userId as string | undefined) ?? "__unknown__";
      if (userId !== currentUser) {
        currentUser = userId;
        count = 0;
      }
      count += 1;
      if (count > 5) toDelete.push(doc.ref);
    }
...
export const syncViewerSessionStats = onDocumentWritten(
  { region: "asia-northeast3", document: "viewerSessions/{uid}/logs/{sessionId}" },
  async (event) => {
    ...
    const afterData = after.data() as DocumentData;
    const beforeData = before?.data() as DocumentData | undefined;
    ...
    if (!isCreate && !isClosedNow) return;
    ...
    await firestore.runTransaction(async (tx) => {
      ...
      const totalsData = totalsSnap.exists ? (totalsSnap.data() as DocumentData) : {};
      ...
      const userData = userSnap.exists ? (userSnap.data() as DocumentData) : {};
...
type MatchEntry = { id: string; score: number; strict?: boolean };
type MatchListing = MatchListingDoc & { id: string };
type MatchBuyer = {
  id: string;
  typePrefs?: string[];
  budgetMin?: number;
...
type BuyerRelevantSnapshot = Omit<MatchBuyer, "id">;
const MATCH_FIELD_KEYS: Array<keyof BuyerRelevantSnapshot> = [
  "typePrefs",
  "budgetMin",
  "budgetMax",
  "monthlyMax",
...
function normalizeViewerTotalsBucket(input?: DocumentData | null): ViewerTotalsBucket {
  return {
...
function buildMatchListingPayload(source: DocumentData): MatchListingDoc {
  return {
...
function shouldIndexListing(source: DocumentData | undefined | null) {
...
function pickBuyerRelevantFields(source?: DocumentData | null): BuyerRelevantSnapshot | null {
  if (!source) return null;
  const snapshot: BuyerRelevantSnapshot = {};
  MATCH_FIELD_KEYS.forEach((key) => {
    const raw = (source as any)[key];
    if (raw == null) return;
    if (key === "typePrefs") {
      snapshot[key] = Array.isArray(raw) ? raw.map((v: any) => String(v)) : undefined;
...
function sameBuyerRelevantFields(a: BuyerRelevantSnapshot | null, b: BuyerRelevantSnapshot | null) {
  return JSON.stringify(a) === JSON.stringify(b);
}

function normalizeBuyerDocument(id: string, data?: DocumentData | null): MatchBuyer | null {
  if (!data) return null;
  const deleted = toOptionalNumber(data.deletedAt);
  if (deleted && deleted > 0) return null;
  const statusText = String(data.status ?? "").toLowerCase();
  if (["archived", "inactive", "완료", "종료"].some((token) => statusText.includes(token))) return null;
  return {
    id,
    typePrefs: Array.isArray(data.typePrefs) ? data.typePrefs.map((v: any) => String(v)) : undefined,
    ...
    areaMinPy: toOptionalNumber((data as any).areaMinPy ?? (data as any).areaMin),
    areaMaxPy: toOptionalNumber((data as any).areaMaxPy ?? (data as any).areaMax),
...
function normalizeListingDocument(id: string, data?: DocumentData | MatchListingDoc | null): MatchListing | null {
  if (!data) return null;
  const deleted = toOptionalNumber((data as any).deletedAt);
  if (deleted && deleted > 0) return null;
  return {
    id,
    ...
    status: typeof data.status === "string" ? data.status : undefined,
    closedByUs: Boolean((data as any).closedByUs),
    deletedAt: deleted,
    updatedAt: toOptionalNumber((data as any).updatedAt),
    ownershipType: (data as any).ownershipType === "partner" ? "partner" : "our",
  };
}

type NormalizedType = "SALE" | "JEONSE" | "WOLSE";

function normalizeListingType(value?: string | null): NormalizedType | undefined {
  const text = String(value ?? "").toLowerCase();
  if (!text) return undefined;
  if (text.includes("sale") || text.includes("매매")) return "SALE";
  if (text.includes("jeonse") || text.includes("전세")) return "JEONSE";
  if (text.includes("rent") || text.includes("월세")) return "WOLSE";
  return undefined;
}

function resolveListingPrice(type: NormalizedType | undefined, listing: MatchListing): number | undefined {
  if (!type) return undefined;
  if (type === "SALE") return listing.price ?? undefined;
  if (type === "JEONSE") return listing.deposit ?? listing.price ?? undefined;
  return listing.monthly ?? listing.deposit ?? listing.price ?? undefined;
}

function buyerAllowsType(buyer: MatchBuyer, type: NormalizedType | undefined) {
...
function buyerAllowsArea(buyer: MatchBuyer, area?: number) {
...
function listingIsActive(listing: MatchListing) {
  const status = String(listing.status ?? "").toLowerCase();
  return !(status.includes("완료") || status.includes("마감") || status.includes("종료"));
}

function passesBasicMatch(buyer: MatchBuyer, listing: MatchListing) {
  if (!listingIsActive(listing)) return false;
  const type = normalizeListingType(listing.type);
  if (!buyerAllowsType(buyer, type)) return false;
  if (!buyerAllowsArea(buyer, listing.area_py)) return false;
  const price = resolveListingPrice(type, listing);
  if (!buyerAllowsBudget(buyer, listing, type, price)) return false;
  return true;
}

function calcMatchScore(buyer: MatchBuyer, listing: MatchListing): number {
  if (!passesBasicMatch(buyer, listing)) return 0;
  let score = 0;
  if (normalizeListingType(listing.type)) score += 1;
  if (typeof listing.area_py === "number") score += 1;
  if (resolveListingPrice(normalizeListingType(listing.type), listing) != null) score += 1;
  return score || 1;
}

function isStrictMatch(buyer: MatchBuyer, listing: MatchListing) {
  const type = normalizeListingType(listing.type);
  if (!type) return false;
  if (!buyer.typePrefs || buyer.typePrefs.length === 0) return false;
  if (!buyer.typePrefs.some((pref) => normalizeListingType(pref) === type)) return false;
...
function matchListingsForBuyer(buyer: MatchBuyer, listings: MatchListing[], limit = MATCH_LIMIT): MatchEntry[] {
  return listings
    .map((listing) => ({
      id: listing.id,
...
function matchBuyersForListing(listing: MatchListing, buyers: MatchBuyer[], limit = MATCH_LIMIT): MatchEntry[] {
  return buyers
    .map((buyer) => ({
      id: buyer.id,
      score: calcMatchScore(buyer, listing),
...
async function loadActiveBuyers(): Promise<MatchBuyer[]> {
  const snap = await firestore.collection("buyers").get();
  return snap.docs
    .map((doc) => normalizeBuyerDocument(doc.id, doc.data()))
    .filter((item): item is MatchBuyer => Boolean(item));
}

async function loadMatchListingDocs(): Promise<MatchListing[]> {
  const snap = await firestore.collection(MATCH_COLLECTION).get();
  return snap.docs
    .map((doc) => normalizeListingDocument(doc.id, doc.data()))
    .filter((item): item is MatchListing => Boolean(item));
}

async function recomputeBuyerSnapshot(
  buyerId: string,
  ctx: { buyers?: MatchBuyer[]; listings?: MatchListing[] } = {}
) {
  ...
  if (!buyer) {
    ...
    return { listingIds: [] as string[] };
  }
  const matches = matchListingsForBuyer(buyer, listings, MATCH_LIMIT);
  await ref.set(
    { buyerId, listingIds: matches.map((m) => m.id), matches, updatedAt: Date.now() },
    { merge: true }
  );
  return { listingIds: matches.map((m) => m.id) };
}

async function recomputeListingSnapshot(
  listingId: string,
  ctx: { listing?: MatchListing | null; buyers?: MatchBuyer[] } = {}
) {
  const buyers = ctx.buyers ?? (await loadActiveBuyers());
  ...
  let listing =
    ctx.listing !== undefined ? ctx.listing : normalizeListingDocument(listingId, snap.exists ? snap.data() : undefined);
  ...
  await ref.set(
    { matchedBuyerIds: matches.map((m) => m.id), matchedBuyers: matches, matchesUpdatedAt: Date.now() },
    { merge: true }
  );
  return { impactedBuyerIds: new Set([...prevBuyerIds, ...matches.map((m) => m.id)]) };
}

export const syncMatchListing = onDocumentWritten(
  { region: "asia-northeast3", document: "listings/{listingId}" },
  async (event) => {
    ...
    if (!shouldIndexListing(after)) {
      ...
      if (prevBuyerIds.length === 0) return;

      const buyers = await loadActiveBuyers();
      const listings = await loadMatchListingDocs();
      await Promise.all(
        prevBuyerIds.map((buyerId) => recomputeBuyerSnapshot(buyerId, { buyers, listings }))
      );
      return;
    }
    ...
    if (impactedBuyerIds.size === 0) return;

    const listings = await loadMatchListingDocs();
    await Promise.all(
      Array.from(impactedBuyerIds).map((buyerId) => recomputeBuyerSnapshot(buyerId, { buyers, listings }))
    );
  }
);

export const syncBuyerMatches = onDocumentWritten(
  { region: "asia-northeast3", document: "buyers/{buyerId}" },
  async (event) => {
    ...
    if (impactedListingIds.size === 0) return;
    const listingMap = new Map(listings.map((row) => [row.id, row]));

    await Promise.all(
      Array.from(impactedListingIds).map((listingId) =>
        recomputeListingSnapshot(listingId, { listing: listingMap.get(listingId) ?? null, buyers })
      )
    );
  }
);
...
export const kakaoAuth = onRequest(
  { region: "asia-northeast3", cors: true, secrets: [KAKAO_REST_KEY, KAKAO_CLIENT_SECRET] },
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!accessToken && code) {
        ...
        const errors: Array<{ redirectUri: string; status: number; detail: string }> = [];
        for (const candidate of redirectCandidates) {
          try {
            ...
            const tokenData = (await tokenResp.json().catch(() => null)) as Record<string, unknown> | null;
      ...
      const providedNickname =
        typeof kakaoProfile.nickname === "string" && kakaoProfile.nickname.trim().length > 0
      ...
      if (!providedNickname || !providedEmail) {
        ...
        if (!providedNickname) missing.push("nickname");
      ...
      if (!record) {
        ...
        if (nickname && record.displayName !== nickname) updates.displayName = nickname;
...
export const rebuildMatchListings = onRequest(
  { region: "asia-northeast3", secrets: [MATCH_REBUILD_KEY], cors: true },
  async (req: Request, res: Response) => {
    try {
      ...
      const normalizedListings: MatchListing[] = [];
      while (true) {
      ...
      const buyers = await loadActiveBuyers();
      const impactedBuyerIds = new Set<string>();
      ...
      await Promise.all(
        Array.from(impactedBuyerIds).map((buyerId) => recomputeBuyerSnapshot(buyerId, { buyers, listings: listingsCache }))
      );
...
export const realpriceSeries = onRequest(
  { region: "asia-northeast3", secrets: [MOLIT_SERVICE_KEY], cors: true },
  async (req: Request, res: Response) => {
    try {
      ...
      const type: Kind = (["jeonse", "wolse", "presale"].includes(kindStr) ? kindStr : "sale") as Kind;
      ...
      const rows: { ym: string; price: number }[] = [];
      for (const ym of ymList) {
        ...
        try {
          ...
          try {
            ...
            const node = (j as any)?.response?.body?.items?.item;
            if (Array.isArray(node)) items = node;
            else if (node) items = [node];
          } catch (e: any) {
          ...
          for (const it of items) {
            ...
            if (complex && !apt.includes(complex)) continue;

            if (type === "sale" || type === "presale") {
              const price = toNumber(it.dealAmount) * 10000;
              if (price > 0) rows.push({ ym, price });
            } else {
              const deposit = toNumber(it.deposit) * 10000;
              const monthly = toNumber(it.rentFee) * 10000;
              if (type === "jeonse") {
                if (monthly === 0 && deposit > 0) rows.push({ ym, price: deposit });
              } else {
                if (monthly > 0) rows.push({ ym, price: deposit + monthly * rentFactor });
              }
            }
          }
        } catch (e: any) {
...
export const realpriceProxy = onRequest(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [meta] = await file.getMetadata().catch(() => [{ contentType: "application/json; charset=utf-8" } as any]);
      res.setHeader("Content-Type", meta?.contentType || "application/json; charset=utf-8");
      if (obj === "daily_latest.json") res.setHeader("Cache-Control", "no-cache");
      else if (meta?.cacheControl) res.setHeader("Cache-Control", meta.cacheControl);
      file
        .createReadStream()
...
function parseDateFromItem(it: any, ym: string): string | undefined {
...
function toPyeong(areaM2?: number): number | undefined {
  if (!areaM2 || !Number.isFinite(areaM2)) return undefined;
  return Math.round(areaM2 / 3.3058);
}
function getAreaM2(it: any): number | undefined {
...
async function buildAliasMap(): Promise<Record<string, string>> {
  try {
    ...
    const raw: any = require("rj-realestate/src/data/complexes.json");
    const list: any[] = Array.isArray(raw) ? raw : raw?.default || [];
    const map: Record<string, string> = {};
    for (const c of list) {
      const name = String(c?.name || "").trim();
      if (!name) continue;
      map[normKey(name)] = name;
      const aliases: any[] = Array.isArray(c?.aliases) ? c.aliases : [];
...
export const buildRealpriceDaily = onSchedule(
  // 매일 00:30 KST 고정 실행
  { schedule: "30 0 * * *", timeZone: "Asia/Seoul", region: "asia-northeast3", secrets: [MOLIT_SERVICE_KEY] },
  async () => {
    await doBuildAndUpload();
  },
);

export const buildRealpriceNow = onRequest(
  { region: "asia-northeast3", secrets: [MOLIT_SERVICE_KEY], cors: true },
...
async function doBuildAndUpload() {
  ...
  for (const ym of ymList) {
    const ymNum = ym.replace("-", "");
    const kinds: Kind[] = ["sale", "jeonse", "wolse", "presale"];
      ...
      // 페이지네이션으로 모든 페이지를 수집하고 바로 다음 종류로 이동
      {
        ...
        while (fetched < totalCount) {
          ...
          try {
            ...
            try {
              const j = JSON.parse(text);
              const body = (j as any)?.response?.body;
            ...
            for (const it of items) {
              ...
              const depositVal =
                type === "sale" || type === "presale" ? toFloat(it.dealAmount) * 10000 : toFloat(it.deposit) * 10000;
              const monthlyVal = type === "sale" || type === "presale" ? 0 : toFloat(it.rentFee) * 10000;
              let price = 0;
              if (type === "sale" || type === "presale") {
                price = depositVal;
              } else if (type === "jeonse") {
                if (monthlyVal === 0 && depositVal > 0) price = depositVal;
              } else {
                if (monthlyVal > 0) price = depositVal + monthlyVal * rentFactor;
              }
              if (price <= 0) continue;
  ...
  // Aggregations
  const monthlyTotalsAll = new Map<string, { total: number; count: number }>();
  const monthlyTotalsByComplex = new Map<string, Map<string, { total: number; count: number }>>();
  const seriesByComplexPyeong = new Map<string, Map<number, Map<string, number[]>>>();
  const monthlyTotalsAllByType = new Map<Kind, Map<string, { total: number; count: number }>>();
  const monthlyTotalsByComplexByType = new Map<string, Map<Kind, Map<string, { total: number; count: number }>>>();
  const seriesByComplexPyeongByType = new Map<string, Map<number, Map<Kind, Map<string, number[]>>>>();

  for (const t of txns) {
    if (!monthlyTotalsAll.has(t.ym)) monthlyTotalsAll.set(t.ym, { total: 0, count: 0 });
    const aggAll = monthlyTotalsAll.get(t.ym)!;
    aggAll.total += t.price;
    aggAll.count += 1;

    if (!monthlyTotalsByComplex.has(t.complex)) monthlyTotalsByComplex.set(t.complex, new Map());
    const byMonth = monthlyTotalsByComplex.get(t.complex)!;
    if (!byMonth.has(t.ym)) byMonth.set(t.ym, { total: 0, count: 0 });
    const agg = byMonth.get(t.ym)!;
    agg.total += t.price;
    agg.count += 1;

    if (!monthlyTotalsAllByType.has(t.type)) monthlyTotalsAllByType.set(t.type, new Map());
    const allByType = monthlyTotalsAllByType.get(t.type)!;
    if (!allByType.has(t.ym)) allByType.set(t.ym, { total: 0, count: 0 });
    const aggAllByType = allByType.get(t.ym)!;
    aggAllByType.total += t.price;
    aggAllByType.count += 1;

    if (!monthlyTotalsByComplexByType.has(t.complex)) monthlyTotalsByComplexByType.set(t.complex, new Map());
    const byCxType = monthlyTotalsByComplexByType.get(t.complex)!;
    if (!byCxType.has(t.type)) byCxType.set(t.type, new Map());
    const byCxTypeYm = byCxType.get(t.type)!;
    if (!byCxTypeYm.has(t.ym)) byCxTypeYm.set(t.ym, { total: 0, count: 0 });
    const aggCxType = byCxTypeYm.get(t.ym)!;
    aggCxType.total += t.price;
    aggCxType.count += 1;

    if (t.pyeong) {
      if (!seriesByComplexPyeong.has(t.complex)) seriesByComplexPyeong.set(t.complex, new Map());
      const byP = seriesByComplexPyeong.get(t.complex)!;
      if (!byP.has(t.pyeong)) byP.set(t.pyeong, new Map());
      const byYm = byP.get(t.pyeong)!;
      if (!byYm.has(t.ym)) byYm.set(t.ym, []);
      byYm.get(t.ym)!.push(t.price);

      if (!seriesByComplexPyeongByType.has(t.complex)) seriesByComplexPyeongByType.set(t.complex, new Map());
      const byP2 = seriesByComplexPyeongByType.get(t.complex)!;
      if (!byP2.has(t.pyeong)) byP2.set(t.pyeong, new Map());
      const byType = byP2.get(t.pyeong)!;
      if (!byType.has(t.type)) byType.set(t.type, new Map());
      const byTypeYm = byType.get(t.type)!;
      if (!byTypeYm.has(t.ym)) byTypeYm.set(t.ym, []);
  ...
  const toSeriesStats = (m: Map<string, number[]>) =>
    [...m.entries()]
      .sort((a, b) => (a[0] < b[0] ? -1 : 1))
      .map(([date, arr]) => {
  ...
  const seriesOut: Record<string, Record<string, { date: string; price: number }[]>> = {};
  ...
  const monthlyTotalsOut: Record<string, { date: string; total: number; count: number }[]> = {
  ...
  const kinds: Kind[] = ["sale", "jeonse", "wolse", "presale"];
  const monthlyTotalsByTypeOut: Record<string, Record<Kind, { date: string; total: number; count: number }[]>> = {
    ALL: { sale: [], jeonse: [], wolse: [], presale: [] } as any,
  };
  for (const k of kinds) {
    const m = monthlyTotalsAllByType.get(k) || new Map();
    (monthlyTotalsByTypeOut.ALL as any)[k] = [...m.entries()]
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([date, v]) => ({ date, total: v.total, count: v.count }));
  }
  for (const [cx, byType] of monthlyTotalsByComplexByType.entries()) {
    const entry: any = { sale: [], jeonse: [], wolse: [], presale: [] };
    for (const k of kinds) {
  ...
  const seriesByTypeOut: Record<string, Record<string, Record<Kind, { date: string; avg: number; median: number; count: number }[]>>> = {};
  for (const [cx, byP] of seriesByComplexPyeongByType.entries()) {
    seriesByTypeOut[cx] = {} as any;
