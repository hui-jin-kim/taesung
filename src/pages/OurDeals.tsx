...
export default function OurDeals() {
  ...
  const { selected, toggle: toggleSelect, isSelected, setMany } = useSelection("ourdeals");
  ...
  const [viewMode, setViewMode] = React.useState<"cards" | "table">(() => {
    try {
      const stored = localStorage.getItem("rj_ourdeals_view");
  ...
  const items = React.useMemo(() => filterAndSort(rows as any[], filters, "ourdeals") as Listing[], [rows, filters]);
  const pagedItems = React.useMemo(() => items.slice(0, visibleCount), [items, visibleCount]);
  const pageUniverse = React.useMemo(() => filterAndSort(rows as any[], makeDefaultFilters(), "ourdeals") as Listing[], [rows]);
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem("rj_ourdeals_view", viewMode);
    } catch {
      /* ignore */
    }
  }, [viewMode]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <header className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex flex-col sm:flex-row sm:items-baseline sm:gap-3">
            <h1 className="text-2xl font-bold">우리 거래</h1>
        ...
        {items.length === 0 ? (
          <div className="text-neutral-500">매물이 없습니다.</div>
        ) : viewMode === "cards" ? (
      ...
      {detailId ? (
        <ErrorBoundary>
          <ListingDetailSheet
            ...
            actions={{ save: true, complete: true }}
          />
        </ErrorBoundary>
      ) : null}
    </div>
